(function () {
  ...
  checkBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        feedback.textContent =
          msg ||
          t("unknownError", "An error occurred while checking availability.");
        return;
      }
  ...
  function renderTablePicker(tables) {
    ...
    if (tables.length === 0) {
      pickerContainer.innerHTML = `<div class="alert alert-light border">${t("noTablesAvailable", "No tables available for the selected time and guest count.")}</div>`;
      return;
    }
  ...
  function updateSubmitState() {
    if (currentSelection) {
      ...
      selectedTableLabel.textContent = `${t("table", "Table")}: ${currentSelection.tableName}`;
      submitBtn.disabled = false;
    } else {
      selectedTableId.value = "";
      selectedTableLabel.textContent = t("selectATable", "Select a table");
